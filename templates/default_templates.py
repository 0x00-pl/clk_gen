...
def format_lines(lines:[list]):
    ...
    for line in lines:
        for item,i in zip(line, range(len(line))):
            max_arr[i] = max(max_arr[i], len(item))
...
def template_module_instance(ast, ty, args, name, ports, comment=None):
    ...
    ret.append(['('])
    module_instance_ports = inc_indentation(template_kv(ports))
    ret.extend(module_instance_ports)
    ret.append([');'])
    return ret
